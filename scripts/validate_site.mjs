...
for (const item of routes) {
  ...
  if (item.name === "cloudflare") {
    ...
    const cloudflareTab = cloudflare.tabs.indexOf("Cloudflare Tunnel");
    const pending = cloudflare.tabs.indexOf("Pendientes");
    const downloads = cloudflare.tabs.indexOf("Descargas");
    if (!required.every(value => cloudflare.text.includes(value)) || cloudflareTab !== red + 1 || pending !== cloudflare.tabs.length - 2 || downloads !== cloudflare.tabs.length - 1 || cloudflare.tabLayout.overflowX || !cloudflare.tabLayout.allVisible) {
