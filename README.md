...
`data/erasmushomes/` es un caché derivado con SHA y fecha; nunca se edita manualmente. El generador también exige el DOCX acordado versionado en `ErasmusHomes/docs/source/`, contrasta su entrada en `SHA256SUMS`, publica esos mismos bytes bajo `docs/downloads/erasmushomes/` y enlaza la fuente Git fijada al mismo SHA que muestra el panel. La copia publicada es siempre generada; no se edita manualmente ni en Replicant Lab ni en Nexus.
